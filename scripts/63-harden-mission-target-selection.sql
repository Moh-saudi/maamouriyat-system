-- ==============================================================================
-- Script 63: Harden mission-target selection in assignment RPC
-- ==============================================================================
BEGIN;
...
BEGIN
  ...
  IF p_source_target_id IS NOT NULL
     AND EXISTS (
       SELECT 1
       FROM public.mission_targets mt
       WHERE mt.id = p_source_target_id
         AND mt.target_type = 'specific_facilities'
     )
     AND EXISTS (
       SELECT 1
  ...
  IF p_source_target_id IS NOT NULL
     AND EXISTS (
       ...
       JOIN public.facilities f
         ON f.id = ANY(v_facility_ids)
       LEFT JOIN public.organizations fo
         ON fo.id = f.organization_id
       WHERE mt.id = p_source_target_id
         AND mt.target_type = 'aggregate'
         AND (
           (mt.scope_level = 'sector' AND NOT (
             mt.sector_id IS NOT NULL
             AND (
               f.sector_id = mt.sector_id
               OR fo.sector_id = mt.sector_id
               OR f.organization_id = mt.sector_id
             )
           ))
           OR
           (mt.scope_level = 'governorate' AND
             f.governorate IS DISTINCT FROM mt.scope_name)
           OR
           (mt.scope_level = 'health_admin' AND NOT (
             f.organization_id = mt.scope_organization_id
             OR f.health_admin = mt.scope_name
           ))
         )
     )
  THEN
    RAISE EXCEPTION 'Selected facility is outside the aggregate mission target scope';
  END IF;
