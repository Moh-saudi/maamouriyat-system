-- ==============================================================================
-- Script 38: Type-aware organization move/reparent
-- ==============================================================================
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Requested organization type is not allowed under selected parent';
  END IF;
  ...
  IF v_new_level + v_max_descendant_depth > 7 THEN
    RAISE EXCEPTION 'Moving this branch would exceed the supported hierarchy depth';
  END IF;
    ...
    SELECT
      ...
      CASE
        ...
        WHEN parent.organization_type_code = 'health_administration'
          THEN COALESCE(NULLIF(BTRIM(parent.new_health_admin), ''), parent.new_health_admin)
  ...
  FROM subtree
  WHERE org.id = subtree.id
    AND org.id <> v_current.id;
