-- Require a verified terminal field outcome for every facility before a
-- grouped assignment can be finalized. Existing terminal rows are not
-- rewritten; the guards apply only to new terminal transitions/finalization.
BEGIN;
...
BEGIN
  ...
  IF NEW.execution_outcome = 'not_performed'
     AND length(BTRIM(COALESCE(NEW.non_execution_reason, ''))) < 5 THEN
    RAISE EXCEPTION
      'MISSION_NON_EXECUTION_REASON_REQUIRED: non-performed outcome requires a reason';
  END IF;
...
DROP TRIGGER IF EXISTS trg_guard_verified_performed_outcome
  ON public.missions;
DROP TRIGGER IF EXISTS trg_guard_verified_terminal_mission_outcome
  ON public.missions;

CREATE TRIGGER trg_guard_verified_terminal_mission_outcome
BEFORE UPDATE ON public.missions
FOR EACH ROW
EXECUTE FUNCTION public.guard_verified_terminal_mission_outcome();
...
DROP TRIGGER IF EXISTS trg_guard_verified_assignment_completion
  ON public.mission_assignment_batches;

CREATE TRIGGER trg_guard_verified_assignment_completion
BEFORE UPDATE OF status ON public.mission_assignment_batches
FOR EACH ROW
EXECUTE FUNCTION public.guard_verified_assignment_completion();
